"""Wraps the ROI-only Retinotopy dataset into full-hemisphere MS-SiT inputs.
...
MSSiTDense expects patch-gridded input over the whole hemisphere and produces
...
model's output is already unpatched back to mesh vertices by MSSiTDense --
...
class MSSiTRetinotopy(Dataset):
    def __init__(self, root, split="Train", hemisphere="Left", prediction="polarAngle",
                 ...
                 ico_mesh=6, ico_grid=4, reorder=True):
        ...
        if not osp.exists(index_path):
            raise FileNotFoundError(
                f"{index_path} not found. MSSiTRetinotopy needs a static LongTensor mapping "
